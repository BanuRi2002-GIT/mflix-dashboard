...
export const getMovies = async () => {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/v1/movies`, {
      method: "GET",
      headers: {
    ...
    // IMPORTANT FIX: return ONLY the array
    return data.movies || [];
  } catch (error) {
...
export const updateMovie = async (movieId, movieDoc) => {
  try {
    ...
    if (result.acknowledged) {
      console.log(`A movie was updated with the _id: ${movieId}`);
      return {
        success: true,
...
export const deleteMovie = async (movieId) => {
  try {
    ...
    if (result.acknowledged) {
      console.log(`A movie was deleted with the _id: ${movieId}`);
      return {
        success: true,
