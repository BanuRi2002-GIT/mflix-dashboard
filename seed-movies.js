...
async function seedDatabase() {
  ...
  try {
    ...
    // Check which collection your app is using (from your code, it's using "movies_n")
    const collectionName = "movies_n";
    
    console.log(`📊 Using collection: ${collectionName}`);
    
    // Clear existing movies
    const deleteResult = await db.collection(collectionName).deleteMany({});
    console.log(`✅ Cleared ${deleteResult.deletedCount} existing movies`);
    
    // Insert sample movies
    const insertResult = await db.collection(collectionName).insertMany(MOVIES);
    ...
    console.log(`   Rating: ${sample[0].imdb.rating}`);
